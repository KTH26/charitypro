...
export const registerServerDataRoutes = (app: Hono<any>) => {
  ...
  app.get('/v3/sola/view', async (c: any) => {
    ...
    const [solaRows,appRows,solaCount,appCount,mappingRows,donorRows]=await c.env.DB.batch([
      ...
      c.env.DB.prepare("SELECT COUNT(*) AS count FROM sync_records WHERE type='transactions' AND is_deleted=0 AND json_extract(data,'$.method')='credit_card' AND json_extract(data,'$.date') BETWEEN ? AND ? AND json_extract(data,'$.type')='pending'").bind(pendingStart,pendingEnd),
      c.env.DB.prepare("SELECT data FROM sync_records WHERE type='solaDonorMappings' AND is_deleted=0"),
      c.env.DB.prepare("SELECT id,data FROM sync_records WHERE type='donors' AND is_deleted=0")
    ]);
    ...
    const donorNames=new Map(donorRows.results.map((row:any)=>{const donor=parseSetting(row.data,{});const english=donor.name||[donor.firstName,donor.lastName].filter(Boolean).join(' ');const yiddish=[donor.preTitle,donor.hebFirstName,donor.hebLastName,donor.title].filter(Boolean).join(' ');return [String(row.id),[english,yiddish].filter(Boolean).join(' · ')];}));
    const autoMatches:any[]=[];const usedTransactions=new Set<string>();for(const match of sola){const mappedDonorId=mappings.get(String(match.name||'').trim().toLowerCase())||'';const solaName=String(match.name||'').toLowerCase();const candidates=appItems.filter((tx:any)=>{if(usedTransactions.has(String(tx.id))||Math.abs(Number(tx.amount||0)-Number(match.amount||0))>=0.005)return false;const names=[tx.donorName,...(Array.isArray(tx.aliases)?tx.aliases:[])].map((name:any)=>String(name).toLowerCase());return (mappedDonorId&&mappedDonorId===String(tx.donorId||''))||names.some(name=>{const first=name.split(' ')[0];const solaFirst=solaName.split(' ')[0];return name===solaName||(first&&solaFirst&&(first.includes(solaFirst)||solaFirst.includes(first)));});}).sort((left:any,right:any)=>Math.abs(new Date(left.date).getTime()-new Date(match.date).getTime())-Math.abs(new Date(right.date).getTime()-new Date(match.date).getTime()));const tx=candidates[0];if(tx){usedTransactions.add(String(tx.id));autoMatches.push({transactionId:tx.id,solaRef:match.ref,...(mappedDonorId?{remembered:true}:{})});}}
