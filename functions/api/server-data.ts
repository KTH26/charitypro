...
export const registerServerDataRoutes = (app: Hono<any>) => {
  ...
  app.get('/v3/sola/view', async (c: any) => {
    ...
    const donorNames=new Map(donorRows.results.map((row:any)=>{const donor=parseSetting(row.data,{});const english=donor.name||[donor.firstName,donor.lastName].filter(Boolean).join(' ');const yiddish=[donor.preTitle,donor.hebFirstName,donor.hebLastName,donor.title].filter(Boolean).join(' ');return [String(row.id),[english,yiddish].filter(Boolean).join(' · ')];}));
    const normalizedName=(value:unknown)=>String(value||'').normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();const autoMatches:any[]=[];const usedTransactions=new Set<string>();for(const match of sola){const mappedDonorId=mappings.get(String(match.name||'').trim().toLowerCase())||'';const solaName=normalizedName(match.name);const candidates=appItems.filter((tx:any)=>{if(usedTransactions.has(String(tx.id))||Math.abs(Number(tx.amountCAD??tx.amount??0)-Number(match.amount||0))>=0.005)return false;const names=[tx.donorName,...(Array.isArray(tx.aliases)?tx.aliases:[])].map(normalizedName);return (mappedDonorId&&mappedDonorId===String(tx.donorId||''))||names.some(name=>{const first=name.split(' ')[0];const solaFirst=solaName.split(' ')[0];return name===solaName||(first&&solaFirst&&(first===solaFirst||first.includes(solaFirst)||solaFirst.includes(first)));});}).sort((left:any,right:any)=>Math.abs(new Date(left.date).getTime()-new Date(match.date).getTime())-Math.abs(new Date(right.date).getTime()-new Date(match.date).getTime()));const tx=candidates[0];if(tx){usedTransactions.add(String(tx.id));autoMatches.push({transactionId:tx.id,solaRef:match.ref,...(mappedDonorId?{remembered:true}:{})});}}
  ...
  app.get('/v3/bank/incoming-transfer-candidates', async (c:any) => {
    ...
    const [transfers,feed]=await c.env.DB.batch([c.env.DB.prepare(`SELECT t.id,t.data,t.revision,t.updated_at,json_extract(a.data,'$.name') AS source_name FROM sync_records t LEFT JOIN sync_records a ON a.type='accounts' AND a.id=json_extract(t.data,'$.fromAccountId') AND a.is_deleted=0 WHERE t.type='accountTransfers' AND t.is_deleted=0 AND json_extract(t.data,'$.toAccountId')=? AND ABS(COALESCE(json_extract(t.data,'$.amount'),0)-?)<0.005 AND COALESCE(json_extract(t.data,'$.destinationBankTransactionId'),'')='' AND date(json_extract(t.data,'$.date')) BETWEEN date(?,'-31 days') AND date(?,'+31 days') ORDER BY ABS(julianday(json_extract(t.data,'$.date'))-julianday(?)),t.updated_at DESC LIMIT 100`).bind(accountId,amount,bankDate,bankDate,bankDate),c.env.DB.prepare(`SELECT f.id,f.data,f.revision,f.updated_at,json_extract(f.data,'$.id') AS bank_transaction_id,json_extract(a.data,'$.name') AS source_name FROM sync_records f LEFT JOIN sync_records a ON a.type='accounts' AND a.id=json_extract(f.data,'$.accountId') AND a.is_deleted=0 WHERE f.type='bankFeedTransactions' AND f.is_deleted=0 AND json_extract(f.data,'$.accountId')<>? AND COALESCE(json_extract(f.data,'$.amount'),0)<0 AND ABS(ABS(json_extract(f.data,'$.amount'))-?)<0.005 AND date(json_extract(f.data,'$.date')) BETWEEN date(?,'-31 days') AND date(?,'+31 days') AND NOT EXISTS(SELECT 1 FROM sync_records m,json_each(m.data) j WHERE m.type='matchedBankTransactions' AND m.is_deleted=0 AND j.value=json_extract(f.data,'$.id')) ORDER BY ABS(julianday(json_extract(f.data,'$.date'))-julianday(?)),f.updated_at DESC LIMIT 100`).bind(accountId,amount,bankDate,bankDate,bankDate)]);
  ...
  app.post('/v3/bank/match-incoming-transfer',async(c:any)=>{
    ...
    const old=parseSetting(current.data,{});if(String(old.toAccountId||'')!==accountId||Math.abs(Number(old.amount||0)-amount)>=0.005||old.destinationBankTransactionId)return c.json({success:false,error:'This transfer no longer matches the credit-card deposit.',conflict:true},409);
