...
export const registerServerDataRoutes = (app: Hono<any>) => {
  ...
  app.post('/v3/sola/sync', async (c: any) => {
    ...
    try {
      const response = await fetch('https://x1.cardknox.com/reportjson',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({xKey:key,xVersion:'5.0.0',xSoftwareName:'CharityApp',xSoftwareVersion:'1.0',xCommand:'report:all',xBeginDate:`${startDate} 00:00:00`,xEndDate:`${endDate} 23:59:59`,xmaxrecords:1000,xgetnewest:false})});
      ...
      const recordsReturned=Number(payload.xRecordsReturned??records.length);const reportingMaximum=Number(payload.xReportingMaxTransactions??1000);const sync={lastSyncAt:now,startDate,endDate,count:records.length,recordsReturned,reportingMaximum,complete:recordsReturned<reportingMaximum||recordsReturned<1000};await c.env.DB.prepare("INSERT INTO sync_metadata(key,value,updated_at) VALUES('sola_sync_status',?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at").bind(JSON.stringify(sync),now).run();
  ...
  app.get('/v3/sola/view', async (c: any) => {
    ...
    const monthStart=(value:string)=>/^\d{4}-\d{2}-\d{2}$/.test(value)?`${value.slice(0,7)}-01`:value;const monthEnd=(value:string)=>{if(!/^\d{4}-\d{2}-\d{2}$/.test(value))return value;const year=Number(value.slice(0,4));const month=Number(value.slice(5,7));return new Date(Date.UTC(year,month,0)).toISOString().slice(0,10);};const pendingStart=monthStart(startDate);const pendingEnd=monthEnd(endDate);
    ...
    const [solaRows,appRows,solaCount,appCount,mappingRows,donorRows]=await c.env.DB.batch([
      c.env.DB.prepare("SELECT s.data FROM sync_records s WHERE s.type='solaTransactions' AND s.is_deleted=0 AND lower(json_extract(s.data,'$.status'))='approved' AND substr(json_extract(s.data,'$.date'),1,10) BETWEEN ? AND ? AND NOT EXISTS (SELECT 1 FROM sync_records p WHERE p.type='processedSolaRefs' AND p.id=s.id AND p.is_deleted=0) ORDER BY json_extract(s.data,'$.date') DESC,s.id DESC LIMIT ? OFFSET ?").bind(startDate,endDate,limit*2,offset),
      c.env.DB.prepare("SELECT t.id,t.data,t.revision,t.updated_at,json_extract(d.data,'$.name') AS donor_name,json_extract(d.data,'$.aliases') AS aliases FROM sync_records t LEFT JOIN sync_records d ON d.type='donors' AND d.id=json_extract(t.data,'$.donorId') AND d.is_deleted=0 WHERE t.type='transactions' AND t.is_deleted=0 AND lower(replace(replace(COALESCE(json_extract(t.data,'$.method'),''),'-','_'),' ','')) IN ('credit_card','creditcard','card') AND json_extract(t.data,'$.date') BETWEEN ? AND ? AND lower(json_extract(t.data,'$.type'))='pending' ORDER BY json_extract(t.data,'$.date') DESC,t.id DESC").bind(pendingStart,pendingEnd),
      c.env.DB.prepare("SELECT COUNT(*) AS count FROM sync_records s WHERE s.type='solaTransactions' AND s.is_deleted=0 AND lower(json_extract(s.data,'$.status'))='approved' AND substr(json_extract(s.data,'$.date'),1,10) BETWEEN ? AND ? AND NOT EXISTS (SELECT 1 FROM sync_records p WHERE p.type='processedSolaRefs' AND p.id=s.id AND p.is_deleted=0)").bind(startDate,endDate),
      c.env.DB.prepare("SELECT COUNT(*) AS count FROM sync_records WHERE type='transactions' AND is_deleted=0 AND lower(replace(replace(COALESCE(json_extract(data,'$.method'),''),'-','_'),' ','')) IN ('credit_card','creditcard','card') AND json_extract(data,'$.date') BETWEEN ? AND ? AND lower(json_extract(data,'$.type'))='pending'").bind(pendingStart,pendingEnd),
      c.env.DB.prepare("SELECT data FROM sync_records WHERE type='solaDonorMappings' AND is_deleted=0"),
      c.env.DB.prepare("SELECT id,data FROM sync_records WHERE type='donors' AND is_deleted=0")
    ]);
    ...
    const donorNames=new Map(donorRows.results.map((row:any)=>{const donor=parseSetting(row.data,{});const english=donor.name||[donor.firstName,donor.lastName].filter(Boolean).join(' ');const yiddish=[donor.preTitle,donor.hebFirstName,donor.hebLastName,donor.title].filter(Boolean).join(' ');return [String(row.id),[english,yiddish].filter(Boolean).join(' · ')];}));
    const normalizedName=(value:unknown)=>String(value||'').normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();const autoMatches:any[]=[];const usedTransactions=new Set<string>();for(const match of sola){const mappedDonorId=mappings.get(String(match.name||'').trim().toLowerCase())||'';const solaName=normalizedName(match.name);const exactAmount=appItems.filter((tx:any)=>!usedTransactions.has(String(tx.id))&&Math.abs(Number(tx.amountCAD??tx.amount??0)-Number(match.amount||0))<0.005).sort((left:any,right:any)=>Math.abs(new Date(left.date).getTime()-new Date(match.date).getTime())-Math.abs(new Date(right.date).getTime()-new Date(match.date).getTime()));const identified=exactAmount.filter((tx:any)=>{const names=[tx.donorName,...(Array.isArray(tx.aliases)?tx.aliases:[])].map(normalizedName);return (mappedDonorId&&mappedDonorId===String(tx.donorId||''))||names.some(name=>{const first=name.split(' ')[0];const solaFirst=solaName.split(' ')[0];return name===solaName||(first&&solaFirst&&(first===solaFirst||first.includes(solaFirst)||solaFirst.includes(first)));});});const tx=identified[0]||(exactAmount.length===1?exactAmount[0]:null);if(tx){usedTransactions.add(String(tx.id));autoMatches.push({transactionId:tx.id,solaRef:match.ref,...(mappedDonorId?{remembered:true}:identified.length===0?{singleExactAmount:true}:{})});}}
  ...
  app.post('/v3/sola/import-new', async (c:any) => {
    ...
    try {
      ...
      const ref=String(body.ref||'').trim();const donorId=String(body.donorId||'').trim();const id=`sola-payment-${encodeURIComponent(ref).slice(0,170)}`;const [priorRows,solaRow,donor,existing]=await c.env.DB.batch([
        c.env.DB.prepare('SELECT result_json FROM processed_mutations WHERE mutation_id=?').bind(mutationId),
        ...
        c.env.DB.prepare("SELECT id,data,revision,updated_at FROM sync_records WHERE type='transactions' AND id=? AND is_deleted=0").bind(id)
      ]);const prior:any=priorRows.results[0];if(prior?.result_json)return c.json(JSON.parse(String(prior.result_json)));const savedSola:any=solaRow.results[0];if(!savedSola)return c.json({success:false,error:'Sola transaction was not found in the saved report.'},404);if(!donor.results[0])return c.json({success:false,error:'Choose a valid donor before importing.'},409);
      if(existing.results[0]){const row:any=existing.results[0];const item={...parseSetting(row.data,{}),id:String(row.id),revision:Number(row.revision),updatedAt:Number(row.updated_at)};const result={success:true,item,ref,existing:true};const now=Date.now();await c.env.DB.batch([c.env.DB.prepare("INSERT INTO sync_records(id,type,data,updated_at,revision,is_deleted,last_operation_id) VALUES(?,'processedSolaRefs',?,?,1,0,?) ON CONFLICT(type,id) DO UPDATE SET data=excluded.data,updated_at=excluded.updated_at,revision=sync_records.revision+1,is_deleted=0,last_operation_id=excluded.last_operation_id").bind(ref,JSON.stringify({ref,transactionId:item.id}),now,mutationId),c.env.DB.prepare('INSERT INTO processed_mutations(mutation_id,result_json,server_time) VALUES(?,?,?)').bind(mutationId,JSON.stringify(result),now)]);return c.json(result);}
      const sola=parseSetting(savedSola.data,{});const date=normalizeExternalDate(sola.date);if(!date)return c.json({success:false,error:'This Sola transaction has an invalid date.'},409);const now=Date.now();const item={id,donorId,amount:Number(sola.amount),amountCAD:Number(sola.amount),date,type:'approved',method:'credit_card',currency:'CAD',depositStatus:'undeposited',sourceAccountId:'sys-undeposited-funds',solaBatchId:sola.batch||'',notes:`Imported as new via Sola Sync. Ref: ${ref}`};const data=JSON.stringify(item);const operationId=`${mutationId}-transaction`;const result={success:true,item:{...item,revision:1,updatedAt:now},ref,existing:false};
      await c.env.DB.batch([
        ...
        c.env.DB.prepare("INSERT INTO sync_records(id,type,data,updated_at,revision,is_deleted,last_operation_id) VALUES(?,'processedSolaRefs',?,?,1,0,?) ON CONFLICT(type,id) DO UPDATE SET data=excluded.data,updated_at=excluded.updated_at,revision=sync_records.revision+1,is_deleted=0,last_operation_id=excluded.last_operation_id").bind(ref,JSON.stringify({ref,transactionId:id}),now,`${mutationId}-processed-ref`),
  ...
  app.get('/v3/bank/incoming-transfer-candidates', async (c:any) => {
    const denied=requirePermission(c,'transactions.read');if(denied)return denied;
    const accountId=String(c.req.query('accountId')||'').trim();const amount=Number(c.req.query('amount'));const bankDate=String(c.req.query('bankDate')||'');
    if(!accountId||!Number.isFinite(amount)||amount<=0||!/^\d{4}-\d{2}-\d{2}$/.test(bankDate))return c.json({success:false,error:'Destination account, amount, and bank date are required.'},400);
    const [transfers,feed]=await c.env.DB.batch([c.env.DB.prepare(`SELECT t.id,t.data,t.revision,t.updated_at,json_extract(a.data,'$.name') AS source_name FROM sync_records t LEFT JOIN sync_records a ON a.type='accounts' AND a.id=json_extract(t.data,'$.fromAccountId') AND a.is_deleted=0 WHERE t.type='accountTransfers' AND t.is_deleted=0 AND json_extract(t.data,'$.fromAccountId')<>? AND ABS(COALESCE(json_extract(t.data,'$.amount'),0)-?)<0.005 AND COALESCE(json_extract(t.data,'$.destinationBankTransactionId'),'')='' ORDER BY ABS(julianday(json_extract(t.data,'$.date'))-julianday(?)),t.updated_at DESC LIMIT 250`).bind(accountId,amount,bankDate),c.env.DB.prepare(`SELECT f.id,f.data,f.revision,f.updated_at,json_extract(f.data,'$.id') AS bank_transaction_id,json_extract(a.data,'$.name') AS source_name FROM sync_records f LEFT JOIN sync_records a ON a.type='accounts' AND a.id=json_extract(f.data,'$.accountId') AND a.is_deleted=0 WHERE f.type='bankFeedTransactions' AND f.is_deleted=0 AND json_extract(f.data,'$.accountId')<>? AND (COALESCE(json_extract(f.data,'$.amount'),0)<0 OR COALESCE(json_extract(f.data,'$.rawAmount'),0)>0) AND ABS(ABS(json_extract(f.data,'$.amount'))-?)<0.005 AND NOT EXISTS(SELECT 1 FROM sync_records m,json_each(m.data) j WHERE m.type='matchedBankTransactions' AND m.is_deleted=0 AND j.value=json_extract(f.data,'$.id')) ORDER BY ABS(julianday(json_extract(f.data,'$.date'))-julianday(?)),f.updated_at DESC LIMIT 250`).bind(accountId,amount,bankDate)]);
    const items=[...transfers.results.map((row:any)=>({...parseRecord(row),candidateKind:'transfer',sourceName:row.source_name||'Bank account'})),...feed.results.map((row:any)=>{const item:any=parseRecord(row);const bankTransactionId=String(row.bank_transaction_id||'');return {...item,id:`feed:${item.accountId}:${bankTransactionId}`,candidateKind:'bankFeed',sourceBankTransactionId:bankTransactionId,sourceAccountId:item.accountId,amount:Math.abs(Number(item.amount)),sourceName:row.source_name||'Bank account'};})];
    return c.json({success:true,items});
  });

  app.post('/v3/bank/match-bank-feed-transfer-pair',async(c:any)=>{
    const denied=requirePermission(c,'transactions.approve');if(denied)return denied;const body=await c.req.json();const requestId=String(c.req.header('Idempotency-Key')||body.requestId||'').trim();if(!requestId)return c.json({success:false,error:'Idempotency-Key is required.'},400);const mutationId=`v3-bank-transfer-pair-${requestId}`;const prior:any=await c.env.DB.prepare('SELECT result_json FROM processed_mutations WHERE mutation_id=?').bind(mutationId).first();if(prior?.result_json)return c.json(JSON.parse(String(prior.result_json)));
    const toAccountId=String(body.accountId||'').trim();const destinationBankTransactionId=String(body.bankTransactionId||'').trim();const fromAccountId=String(body.sourceAccountId||'').trim();const sourceBankTransactionId=String(body.sourceBankTransactionId||'').trim();const amount=Number(body.amount);if(!toAccountId||!destinationBankTransactionId||!fromAccountId||!sourceBankTransactionId||fromAccountId===toAccountId||!Number.isFinite(amount)||amount<=0)return c.json({success:false,error:'Both bank transactions, two different accounts, and a positive amount are required.'},400);
    const [sourceRow,matchRow,linkedRows]=await c.env.DB.batch([c.env.DB.prepare("SELECT data FROM sync_records WHERE type='bankFeedTransactions' AND is_deleted=0 AND json_extract(data,'$.accountId')=? AND json_extract(data,'$.id')=? LIMIT 1").bind(fromAccountId,sourceBankTransactionId),c.env.DB.prepare("SELECT data,revision FROM sync_records WHERE type='matchedBankTransactions' AND id='matchedBankTransactions' AND is_deleted=0"),c.env.DB.prepare("SELECT id FROM sync_records WHERE is_deleted=0 AND (COALESCE(json_extract(data,'$.bankTransactionId'),'') IN (?,?) OR COALESCE(json_extract(data,'$.destinationBankTransactionId'),'') IN (?,?)) LIMIT 1").bind(sourceBankTransactionId,destinationBankTransactionId,sourceBankTransactionId,destinationBankTransactionId)]);const source:any=sourceRow.results[0];const match:any=matchRow.results[0];if(!source)return c.json({success:false,error:'The outgoing bank payment was not found.'},404);if(!match)return c.json({success:false,error:'Bank match history is unavailable.'},500);if(linkedRows.results[0])return c.json({success:false,error:'One of these bank transactions is already linked.'},409);const sourceData=parseSetting(source.data,{});const isOutgoing=Number(sourceData.amount)<0||Number(sourceData.rawAmount)>0;if(!isOutgoing||Math.abs(Math.abs(Number(sourceData.amount))-amount)>=0.005)return c.json({success:false,error:'The outgoing bank payment amount no longer matches.',conflict:true},409);const matchedIds:string[]=Array.isArray(parseSetting(match.data,[]))?parseSetting(match.data,[]):[];if(matchedIds.includes(sourceBankTransactionId)||matchedIds.includes(destinationBankTransactionId))return c.json({success:false,error:'One of these bank transactions is already matched.'},409);
    const now=Date.now();const id=crypto.randomUUID();const record={id,fromAccountId,toAccountId,amount,date:String(sourceData.date||body.bankDate||'').slice(0,10),notes:`Credit-card payment: ${sourceData.description||'Bank payment'}`,bankTransactionId:sourceBankTransactionId,destinationBankTransactionId};const data=JSON.stringify(record);const revision=Number(match.revision)+1;const nextMatchedData=JSON.stringify([...new Set([...matchedIds,sourceBankTransactionId,destinationBankTransactionId])]);const operationId=`${mutationId}-transfer`;const matchOperationId=`${mutationId}-match`;const response={success:true,item:{...record,revision:1,updatedAt:now}};const userId=String(c.get('userId')||'unknown');const userEmail=String(c.get('userEmail')||'unknown');try{await c.env.DB.batch([c.env.DB.prepare("INSERT INTO sync_records(id,type,data,updated_at,revision,is_deleted,last_operation_id) VALUES(?,'accountTransfers',?,?,1,0,?)").bind(id,data,now,operationId),c.env.DB.prepare("INSERT INTO sync_changes(record_id,type,revision,operation,data,changed_at,mutation_id,operation_id) VALUES(?,'accountTransfers',1,'insert',?,?,?,?)").bind(id,data,now,mutationId,operationId),c.env.DB.prepare("INSERT INTO audit_log(record_id,record_type,action,old_revision,new_revision,old_data,new_data,changed_by_user_id,changed_by_email,changed_at,mutation_id,operation_id,reason) VALUES(?,'accountTransfers','insert',NULL,1,NULL,?,?,?,?,?,?,?)").bind(id,data,userId,userEmail,now,mutationId,operationId,'Matched outgoing bank payment to incoming credit-card deposit'),c.env.DB.prepare("UPDATE sync_records SET data=?,revision=?,updated_at=?,last_operation_id=? WHERE type='matchedBankTransactions' AND id='matchedBankTransactions' AND revision=? AND is_deleted=0").bind(nextMatchedData,revision,now,matchOperationId,match.revision),c.env.DB.prepare("INSERT INTO sync_changes(record_id,type,revision,operation,data,changed_at,mutation_id,operation_id) SELECT id,type,revision,'update',data,updated_at,?,? FROM sync_records WHERE type='matchedBankTransactions' AND id='matchedBankTransactions' AND revision=?").bind(mutationId,matchOperationId,revision),c.env.DB.prepare('INSERT INTO processed_mutations(mutation_id,result_json,server_time) VALUES(?,?,?)').bind(mutationId,JSON.stringify(response),now)]);return c.json(response);}catch{const repeated:any=await c.env.DB.prepare('SELECT result_json FROM processed_mutations WHERE mutation_id=?').bind(mutationId).first();if(repeated?.result_json)return c.json(JSON.parse(String(repeated.result_json)));return c.json({success:false,error:'The bank transactions changed. Reload and try again.',conflict:true},409);}
  });

  app.post('/v3/bank/match-incoming-transfer',async(c:any)=>{
    const denied=requirePermission(c,'transactions.approve');if(denied)return denied;const body=await c.req.json();const requestId=String(c.req.header('Idempotency-Key')||body.requestId||'').trim();if(!requestId)return c.json({success:false,error:'Idempotency-Key is required.'},400);
    const mutationId=`v3-bank-incoming-transfer-${requestId}`;const prior:any=await c.env.DB.prepare('SELECT result_json FROM processed_mutations WHERE mutation_id=?').bind(mutationId).first();if(prior?.result_json)return c.json(JSON.parse(String(prior.result_json)));
    const accountId=String(body.accountId||'').trim();const bankTransactionId=String(body.bankTransactionId||'').trim();const transferId=String(body.transferId||'').trim();const amount=Number(body.amount);const expectedRevision=Number(body.revision);
    if(!accountId||!bankTransactionId||!transferId||!Number.isFinite(amount)||amount<=0||!Number.isInteger(expectedRevision))return c.json({success:false,error:'Deposit, transfer, amount, and revision are required.'},400);
    const [transferRow,matchRow,linkedRow]=await c.env.DB.batch([c.env.DB.prepare("SELECT data,revision FROM sync_records WHERE type='accountTransfers' AND id=? AND is_deleted=0").bind(transferId),c.env.DB.prepare("SELECT data,revision FROM sync_records WHERE type='matchedBankTransactions' AND id='matchedBankTransactions' AND is_deleted=0"),c.env.DB.prepare("SELECT id FROM sync_records WHERE is_deleted=0 AND COALESCE(json_extract(data,'$.bankTransactionId'),'')=? OR is_deleted=0 AND COALESCE(json_extract(data,'$.destinationBankTransactionId'),'')=? LIMIT 1").bind(bankTransactionId,bankTransactionId)]);
    const current:any=transferRow.results[0];const match:any=matchRow.results[0];if(!current)return c.json({success:false,error:'Account transfer not found.'},404);if(!match)return c.json({success:false,error:'Bank match history is unavailable.'},500);if(linkedRow.results[0])return c.json({success:false,error:'This credit-card deposit is already matched.'},409);if(Number(current.revision)!==expectedRevision)return c.json({success:false,error:'This transfer changed. Reload the list and try again.',conflict:true},409);
    const old=parseSetting(current.data,{});if(String(old.fromAccountId||'')===accountId||Math.abs(Number(old.amount||0)-amount)>=0.005||old.destinationBankTransactionId)return c.json({success:false,error:'This transfer no longer matches the credit-card deposit.',conflict:true},409);
    const matchedIds:string[]=Array.isArray(parseSetting(match.data,[]))?parseSetting(match.data,[]):[];if(matchedIds.includes(bankTransactionId))return c.json({success:false,error:'This credit-card deposit is already matched.'},409);
    const now=Date.now();const next={...old,toAccountId:accountId,destinationBankTransactionId:bankTransactionId};const data=JSON.stringify(next);const revision=expectedRevision+1;const nextMatchedData=JSON.stringify([...new Set([...matchedIds,bankTransactionId])]);const nextMatchRevision=Number(match.revision)+1;const operationId=`${mutationId}-transfer`;const matchOperationId=`${mutationId}-match`;const response={success:true,item:{...next,id:transferId,revision,updatedAt:now}};const userId=String(c.get('userId')||'unknown');const userEmail=String(c.get('userEmail')||'unknown');
    try{await c.env.DB.batch([c.env.DB.prepare("UPDATE sync_records SET data=?,revision=?,updated_at=?,last_operation_id=? WHERE type='accountTransfers' AND id=? AND revision=? AND is_deleted=0").bind(data,revision,now,operationId,transferId,expectedRevision),c.env.DB.prepare("INSERT INTO sync_changes(record_id,type,revision,operation,data,changed_at,mutation_id,operation_id) VALUES(?,'accountTransfers',?,'update',?,?,?,?)").bind(transferId,revision,data,now,mutationId,operationId),c.env.DB.prepare("INSERT INTO audit_log(record_id,record_type,action,old_revision,new_revision,old_data,new_data,changed_by_user_id,changed_by_email,changed_at,mutation_id,operation_id,reason) VALUES(?,'accountTransfers','update',?,?,?,?,?,?,?,?,?,?)").bind(transferId,expectedRevision,revision,current.data,data,userId,userEmail,now,mutationId,operationId,'Linked credit-card deposit to existing bank transfer'),c.env.DB.prepare("UPDATE sync_records SET data=?,revision=?,updated_at=?,last_operation_id=? WHERE type='matchedBankTransactions' AND id='matchedBankTransactions' AND revision=? AND is_deleted=0").bind(nextMatchedData,nextMatchRevision,now,matchOperationId,match.revision),c.env.DB.prepare("INSERT INTO sync_changes(record_id,type,revision,operation,data,changed_at,mutation_id,operation_id) SELECT id,type,revision,'update',data,updated_at,?,? FROM sync_records WHERE type='matchedBankTransactions' AND id='matchedBankTransactions' AND revision=?").bind(mutationId,matchOperationId,nextMatchRevision),c.env.DB.prepare('INSERT INTO processed_mutations(mutation_id,result_json,server_time) VALUES(?,?,?)').bind(mutationId,JSON.stringify(response),now)]);return c.json(response);}catch{const repeated:any=await c.env.DB.prepare('SELECT result_json FROM processed_mutations WHERE mutation_id=?').bind(mutationId).first();if(repeated?.result_json)return c.json(JSON.parse(String(repeated.result_json)));return c.json({success:false,error:'The transfer or bank feed changed. Reload and try again.',conflict:true},409);}
  });
