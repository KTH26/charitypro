...
export const OnlineExpenses: React.FC = () => {
  ...
  const saveExpense = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch(editingBill ? `/api/v3/records/bills/${encodeURIComponent(editingBill.id)}` : '/api/v3/bills', { method: editingBill ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': requestId }, body: JSON.stringify(editingBill ? { revision: editingBill.revision, data: { ...form, amount } } : { ...form, amount, requestId }) });
      const data = await response.json();
      if (!response.ok || !data.success) { createRequestId.current = ''; throw new Error(data.error || 'Expense could not be saved.'); }
      createRequestId.current = ''; setShowAdd(false); setEditingBill(null); setSelectedBill(null); setNotice(editingBill ? 'Expense changes saved directly to the cloud.' : 'Expense saved directly to the cloud.'); setStatus('open'); setPage(1); setForm(current => ({ ...current, vendor: '', amount: '', memo: '' })); void load();
  ...
  const openNew = () => { setEditingBill(null); setForm({ vendor: '', amount: '', currency: 'CAD', dueDate: new Date().toISOString().slice(0, 10), category: '', status: 'pending', memo: '', taxable: false }); setShowAdd(true); setNotice(''); };
  const openEdit = (bill: Bill) => { setEditingBill(bill); setSelectedBill(null); setForm({ vendor: bill.vendor, amount: String(bill.amount), currency: bill.currency || 'CAD', dueDate: bill.dueDate, category: bill.category, status: bill.status, memo: bill.memo || '', taxable: Boolean(bill.taxable) }); setShowAdd(true); setNotice(''); };
  ...
  return <main style={{ minHeight: '100vh', background: 'var(--bg)', padding: 28, fontFamily: 'Inter, sans-serif' }}><div style={{ maxWidth: 1400, margin: '0 auto' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 20, alignItems: 'center', marginBottom: 22 }}><div><div style={{ color: 'var(--green)', fontWeight: 800, fontSize: 13 }}>CHARITYPRO CLOUD</div><h1 style={{ color: 'var(--navy)', margin: '4px 0' }}>Expenses ({total.toLocaleString()})</h1><div style={{ color: 'var(--green)', fontWeight: 700 }}>{status === 'open' ? 'Open' : 'Paid'} total: ${totalCAD.toLocaleString('en-CA', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} CAD</div><div style={{ color: 'var(--text-muted)', fontSize: 13 }}>Updates automatically every 3 seconds.</div></div>
      <button className="btn btn-primary" onClick={openNew}>Add Expense</button></div>

    {showAdd && <div className="modal-overlay" onClick={() => { setShowAdd(false); setEditingBill(null); }}><section className="modal" onClick={event => event.stopPropagation()} style={{ maxWidth: 760, width: '92%', maxHeight: '90vh', overflowY: 'auto' }}><div className="modal-header"><div><h2 style={{ margin: 0, color: 'var(--navy)' }}>{editingBill ? `Edit Bill — ${editingBill.vendor}` : 'Add New Bill'}</h2><div style={{ color: 'var(--text-muted)' }}>Changes save directly to the shared cloud.</div></div><button className="modal-close" onClick={() => { setShowAdd(false); setEditingBill(null); }}>×</button></div><form className="modal-body" onSubmit={saveExpense}><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 14 }}>
    ...
    </div><label className="form-group" style={{ margin: '14px 0 0' }}><span>Memo</span><textarea rows={2} maxLength={2000} value={form.memo} onChange={e => setField('memo', e.target.value)} /></label><label style={{ display: 'flex', gap: 8, marginTop: 12 }}><input type="checkbox" checked={form.taxable} onChange={e => setField('taxable', e.target.checked)} /> Taxable</label><div className="modal-footer" style={{ margin: '16px -22px -22px', padding: '16px 22px' }}><button type="button" className="btn btn-secondary" onClick={() => { setShowAdd(false); setEditingBill(null); }}>Cancel</button><button className="btn btn-primary" disabled={saving}>{saving ? 'Saving securely...' : editingBill ? 'Save Changes' : 'Save Expense'}</button></div></form></section></div>}

    {paying && <div className="modal-overlay" onClick={() => { setPaying(null); setPaySource(''); }}><section className="modal" onClick={event => event.stopPropagation()} style={{ maxWidth: 560 }}><div className="modal-header"><h2 style={{ color: 'var(--navy)', margin: 0 }}>Mark {paying.vendor} Paid</h2><button className="modal-close" onClick={() => { setPaying(null); setPaySource(''); }}>×</button></div><div className="modal-body"><p>{paying.currency || 'CAD'} ${paying.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })} allocated to {paying.categoryName}</p><label className="form-group"><span>Paid from *</span><select value={paySource} onChange={e => setPaySource(e.target.value)}><option value="">Select asset or liability account</option>{paymentAccounts.map(account => <option key={account.id} value={account.id}>{account.name} ({account.currency})</option>)}</select></label></div><div className="modal-footer"><button className="btn btn-secondary" onClick={() => { setPaying(null); setPaySource(''); }}>Cancel</button><button className="btn btn-primary" disabled={!paySource || saving} onClick={() => void markPaid()}>{saving ? 'Saving securely...' : 'Confirm Paid'}</button></div></section></div>}
