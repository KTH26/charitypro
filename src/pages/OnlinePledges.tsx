...
type Pledge = { id: string; revision: number; donorId: string; donorName: string; donorHebrewName?: string; amount: number; currency: 'CAD' | 'USD'; date: string; notes?: string; paid: number; pending: number; scheduled: number; balance: number };
...
export const OnlinePledges: React.FC = () => {
  ...
  return <main style={{ minHeight: '100vh', background: 'var(--bg)', padding: 28, fontFamily: 'Inter, sans-serif' }}><div style={{ maxWidth: 1400, margin: '0 auto' }}>
    ...
    {editing && <div className="modal-overlay" onClick={() => setEditing(false)}><form className="modal" onClick={event => event.stopPropagation()} onSubmit={save} style={{ maxWidth: 760, width: '92%' }}><div className="modal-header"><h2 style={{ margin: 0, color: 'var(--navy)' }}>{form.id ? 'Edit Pledge' : 'New Pledge'}</h2><button type="button" className="modal-close" onClick={() => setEditing(false)}>×</button></div><div className="modal-body"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 14 }}>
    ...
    <section className="card" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      {loading ? <div style={{ padding: 40, textAlign: 'center' }}>Loading pledges...</div> : <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>
        <SortableTh column="date" label="Date" sort={sort} direction={direction} onSort={changeSort}/><SortableTh column="donor" label="Donor" sort={sort} direction={direction} onSort={changeSort}/><th>Yiddish name</th><SortableTh column="amount" label="Pledge" sort={sort} direction={direction} onSort={changeSort} align="right"/><th style={{textAlign:'right'}}>Paid</th><th style={{textAlign:'right'}}>Scheduled</th><th style={{textAlign:'right'}}>Open balance</th><th />
      </tr></thead><tbody>{items.map(item => <tr key={item.id} onClick={() => setSelectedPledgeId(item.id)} style={{ cursor: 'pointer' }}><td>{item.date}</td><td style={{ fontWeight: 800 }}>{item.donorName}</td><td dir="rtl" style={{fontWeight:700}}>{item.donorHebrewName||'—'}</td><td style={{ textAlign: 'right', fontWeight: 800 }}>{item.currency} ${item.amount.toFixed(2)}</td><td style={{textAlign:'right'}}>${Number(item.paid||0).toFixed(2)}</td><td style={{textAlign:'right'}}>${Number(item.scheduled||0).toFixed(2)}</td><td style={{textAlign:'right',fontWeight:900,color:Number(item.balance||0)>0?'var(--red)':'var(--green)'}}>${Number(item.balance||0).toFixed(2)}</td><td style={{ textAlign: 'right' }}><button className="btn btn-secondary btn-sm" onClick={event => { event.stopPropagation(); beginEdit(item); }}>Edit</button> <button className="btn btn-danger btn-sm" onClick={event => { event.stopPropagation(); void remove(item); }}>Delete</button></td></tr>)}{items.length === 0 && <tr><td colSpan={8} style={{ padding: 30, textAlign: 'center' }}>No matching pledges.</td></tr>}</tbody></table></div>}
