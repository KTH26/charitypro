...
export const OnlinePledges: React.FC = () => {
  ...
  return <main style={{ minHeight: '100vh', background: 'var(--bg)', padding: 28, fontFamily: 'Inter, sans-serif' }}><div style={{ maxWidth: 1400, margin: '0 auto' }}>
    ...
    <section className="card" style={{ padding: 0, overflow: 'hidden' }}><div style={{ padding: 16, borderBottom: '1px solid var(--border)' }}><input value={search} onChange={event => { setSearch(event.target.value); setPage(1); }} placeholder="Search donor or notes" /></div>{loading ? <div style={{ padding: 40, textAlign: 'center' }}>Loading pledges...</div> : <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr><th>Date</th><th>Donor</th><th>Notes</th><th style={{ textAlign: 'right' }}>Amount</th><th /></tr></thead><tbody>{items.map(item => <tr key={item.id}><td>{item.date}</td><td style={{ fontWeight: 800 }}>{item.donorName}</td><td>{item.notes || ''}</td><td style={{ textAlign: 'right', fontWeight: 800 }}>{item.currency} ${item.amount.toFixed(2)}</td><td style={{ textAlign: 'right' }}><button className="btn btn-secondary btn-sm" onClick={() => beginEdit(item)}>Edit</button> <button className="btn btn-danger btn-sm" onClick={() => void remove(item)}>Delete</button></td></tr>)}{items.length === 0 && <tr><td colSpan={5} style={{ padding: 30, textAlign: 'center' }}>No matching pledges.</td></tr>}</tbody></table></div>}<div style={{ display: 'flex', justifyContent: 'space-between', padding: 14, borderTop: '1px solid var(--border)' }}><span>Page {page} of {totalPages}</span><div style={{ display: 'flex', gap: 8 }}><button className="btn btn-secondary btn-sm" disabled={page <= 1 || loading} onClick={() => setPage(value => Math.max(1, value - 1))}>Previous</button><button className="btn btn-secondary btn-sm" disabled={page >= totalPages || loading} onClick={() => setPage(value => Math.min(totalPages, value + 1))}>Next</button></div></div></section>
  </div></main>;
};
