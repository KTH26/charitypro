              ...
              <div>
                ...
                <div className="card" style={{ padding: '24px', border: '1px solid rgba(239,68,68,0.3)', background: 'var(--red-bg)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', marginTop: '24px' }}>
                    ...
                    <div>
                      <h3 style={{ margin: 0, fontSize: '1.2rem', fontFamily: 'Outfit, sans-serif', color: 'var(--blue)' }}>Initialize Real-Time Sync (One-Time)</h3>
                      <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Click this ONLY ONCE on the computer with the most up-to-date data. This breaks your database into individual records and seeds the multi-device sync engine in the cloud.</div>
                    </div>
                  </div>
                  <button 
                    className="btn btn-secondary" 
                    style={{ width: '100%', padding: '12px', fontWeight: 600, borderColor: 'var(--blue)', color: 'var(--blue)' }}
                    onClick={async () => {
                      if (window.confirm('WARNING: Are you sure this computer has the most correct and up-to-date information? This will overwrite the V2 cloud sync database with your current local data.')) {
                        try {
                          const { migrateToCloud } = await import('../components/SyncEngine');
                          const result = await migrateToCloud();
                          alert(`Successfully initialized cloud sync! Uploaded ${result.count} records. Your partner can now safely use the app.`);
                        } catch (e: any) {
                          alert(`Failed to initialize sync: ${e.message || e}`);
                        }
                      }
                    }}
                  >
                    Migrate Database to V2 Sync
                  </button>
                </div>
              </div>
