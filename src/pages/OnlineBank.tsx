...
type BankAccount = { id: string; name: string; currency: string; bankConnected: boolean; savedTransactionCount?: number };
...
export const OnlineBank: React.FC = () => {
  ...
  const syncFeed = async () => {
    ...
    try { const response = await fetch('/api/plaid/transactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accountId: selectedBank, startDate: startDate || undefined }) }); const data = await response.json(); if (!response.ok || data.error) throw new Error(data.details ? (JSON.parse(data.details).error_message || data.error) : (data.error || 'Unable to sync bank transactions.')); setFeedPage(1); setStartDate(''); setNotice(`Bank feed updated from ${data.sync?.fetchedFrom || 'the last sync'} through ${data.sync?.fetchedTo || 'today'}. ${data.transactions?.length || 0} transactions checked and saved in the cloud.`); await loadFeed(true); }
    catch (e: any) { setError(e.message || 'Unable to sync bank transactions.'); } finally { setLoading(false); }
  };
  ...
  const loadDepositCandidates=async(transaction:BankTransaction,from?:string,to?:string)=>{const params=new URLSearchParams({bankDate:transaction.date,...(from?{from}:{}),...(to?{to}:{})});const response=await fetch(`/api/v3/bank/deposit-candidates?${params}`);const data=await response.json();if(!response.ok||!data.success)throw new Error(data.error||'Unable to load deposit candidates.');setCandidates(data.items);setCandidateWindow({startDate:data.startDate,endDate:data.endDate});};
  ...
  const matchDepositPayments = async (transactionIds: string[]) => {
    ...
    try {
      const response = await fetch('/api/v3/bank/match-deposit', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': requestId }, body: JSON.stringify({ requestId, accountId: selectedBank, bankTransactionId: matching.id, bankDate: matching.date, candidateFrom:candidateWindow.startDate,candidateTo:candidateWindow.endDate,description: matching.description, amount: matching.amount, transactionIds }) });
      const data = await response.json();
      ...
      const matchedId=matching.id;delete matchRequestIds.current[matchedId];removeMatchedFromVisibleFeed(matchedId);setMatching(null); setCandidates([]); setSelectedIds([]); setNotice(`Deposit matched to ${data.selectedCount} cloud payment${data.selectedCount === 1 ? '' : 's'}.`); await Promise.all([loadState(true),loadFeed(true)]);
  ...
  const openOutgoingMatch = async (transaction: BankTransaction) => {
    setOutgoing(transaction); setOutgoingAction('expense'); setShowFullExpense(true); setVendor(transaction.description); setCategory(''); setTaxable(false); setTargetAccountId(''); setBillId(''); setBillCandidates([]); setError(''); setNotice('');
    try {
  ...
  const confirmOutgoing = async () => {
    ...
    try {
      ...
      const matchedId=outgoing.id;delete matchRequestIds.current[matchedId];removeMatchedFromVisibleFeed(matchedId);setOutgoing(null); setNotice(data.action === 'expense' ? 'Bank transaction recorded as a paid cloud expense.' : data.action === 'existing_bill' ? 'Bank transaction linked to the existing cloud bill.' : 'Bank transaction recorded as a cloud account transfer.'); await Promise.all([loadState(true),loadFeed(true)]);
  ...
  return <main style={{ minHeight: '100vh', background: 'var(--bg)', padding: 28, fontFamily: 'Inter, sans-serif' }}><div style={{ maxWidth: 1400, margin: '0 auto' }}>
    <div style={{ marginBottom: 22 }}><div style={{ color: 'var(--green)', fontWeight: 800, fontSize: 13 }}>CHARITYPRO CLOUD</div><h1 style={{ color: 'var(--navy)', margin: '4px 0' }}>Bank Matching</h1><div style={{ color: 'var(--text-muted)' }}>Plaid feed with cloud-owned match history. Match state updates automatically every 3 seconds.</div></div>
    ...
    {accounts.length > 0 && <section className="card" style={{ padding: 18, marginBottom: 18 }}><div style={{display:'flex',gap:8,overflowX:'auto',paddingBottom:14,marginBottom:14,borderBottom:'1px solid var(--border)'}}>{accounts.map(account=><button key={account.id} className={`btn ${selectedBank===account.id?'btn-primary':'btn-secondary'}`} style={{whiteSpace:'nowrap',display:'grid',gap:2,textAlign:'left'}} onClick={()=>{setSelectedBank(account.id);setFeedPage(1);setSearch('');setFilterFrom('');setFilterTo('');}}><strong>{account.name}</strong><small>{Number(account.savedTransactionCount||0).toLocaleString()} saved transaction{Number(account.savedTransactionCount||0)===1?'':'s'}{account.bankConnected?'':' · reconnect required'}</small></button>)}</div><div style={{ display: 'grid', gridTemplateColumns: 'minmax(190px,1fr) auto', gap: 10 }}><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} title="Optional: choose an earlier date for this sync" /><button className="btn btn-primary" onClick={() => void syncFeed()} disabled={loading || !selectedAccount?.bankConnected}>{loading ? 'Syncing...' : `Sync New Transactions for ${selectedAccount?.name||'Bank'}`}</button></div><div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>Each bank keeps its own saved feed. Transactions load automatically when you open a bank tab. Sync only checks for newer transactions, starting from {startDate ? `your selected date (${startDate})` : lastSyncDate ? `the last successful sync (${lastSyncDate})` : 'the last 30 days for this first sync'}.</div></section>}
    {matching && <div className="modal-overlay" onClick={()=>{setMatching(null);setSelectedIds([]);}}><section className="card" onClick={event=>event.stopPropagation()} style={{padding:22,border:'2px solid var(--green)',width:'min(920px,94vw)',maxHeight:'90vh',overflowY:'auto',background:'#fff',boxShadow:'0 24px 70px rgba(15,23,42,.3)'}}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 16 }}><div><h2 style={{ color: 'var(--navy)', margin: 0 }}>Match deposit: ${matching.amount.toFixed(2)}</h2><div style={{ color: 'var(--text-muted)', marginTop: 4 }}>{matching.date} · {matching.description}</div></div><button className="btn btn-ghost" onClick={() => { setMatching(null); setSelectedIds([]); }}>Close</button></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr auto',gap:8,alignItems:'end',margin:'16px 0',padding:12,background:'var(--bg-input)',borderRadius:10}}><label className="form-group" style={{margin:0}}><span>Payment date from</span><input type="date" value={candidateWindow.startDate} onChange={event=>setCandidateWindow(current=>({...current,startDate:event.target.value}))}/></label><label className="form-group" style={{margin:0}}><span>Payment date to</span><input type="date" value={candidateWindow.endDate} onChange={event=>setCandidateWindow(current=>({...current,endDate:event.target.value}))}/></label><button className="btn btn-secondary" onClick={()=>void loadDepositCandidates(matching,candidateWindow.startDate,candidateWindow.endDate)}>Search Dates</button></div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,margin:'0 0 12px',flexWrap:'wrap'}}><p style={{margin:0}}>Showing eligible Undeposited Funds payments from <strong>{candidateWindow.startDate}</strong> through <strong>{candidateWindow.endDate}</strong>.</p><div style={{display:'flex',gap:8}}><button className="btn btn-primary btn-sm" disabled={!candidates.length||selectedIds.length===candidates.length} onClick={()=>setSelectedIds(candidates.map(candidate=>candidate.id))}>Select All ({candidates.length})</button><button className="btn btn-secondary btn-sm" disabled={!selectedIds.length} onClick={()=>setSelectedIds([])}>Clear All</button></div></div><div style={{ maxHeight: 390, overflow: 'auto', border: '1px solid var(--border)', borderRadius: 10 }}><table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr><th style={{ width: 40 }}><input type="checkbox" aria-label="Select all payments" checked={candidates.length>0&&selectedIds.length===candidates.length} onChange={event=>setSelectedIds(event.target.checked?candidates.map(candidate=>candidate.id):[])}/></th><th>Date</th><th>Donor</th><th>Method</th><th style={{ textAlign: 'right' }}>Amount CAD</th></tr></thead><tbody>{candidates.map(candidate => <tr key={candidate.id}><td><input type="checkbox" checked={selectedIds.includes(candidate.id)} onChange={() => setSelectedIds(ids => ids.includes(candidate.id) ? ids.filter(id => id !== candidate.id) : [...ids, candidate.id])} /></td><td>{candidate.date}</td><td>{candidate.donorName}</td><td>{candidate.method}</td><td style={{ textAlign: 'right', fontWeight: 800 }}>${Number(candidate.amountCAD ?? candidate.amount).toFixed(2)}</td></tr>)}{candidates.length === 0 && <tr><td colSpan={5} style={{ padding: 25, textAlign: 'center' }}><div style={{marginBottom:10}}>No eligible payments in this date window.</div><button className="btn btn-primary" onClick={()=>setShowMissingPayment(true)}>Enter Missing Payment</button></td></tr>}</tbody></table></div><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}><button className="btn btn-secondary" onClick={()=>setShowMissingPayment(true)}>+ Enter New Payment</button><div><strong>{selectedIds.length} selected · ${selectedTotal.toFixed(2)}</strong><span style={{ color: totalsMatch ? 'var(--green)' : 'var(--red)', marginLeft: 12 }}>{totalsMatch ? 'Exact match' : `Difference: $${(matching.amount - selectedTotal).toFixed(2)}`}</span></div><button className="btn btn-primary" disabled={!totalsMatch || selectedIds.length === 0 || saving} onClick={() => void confirmDeposit()}>{saving ? 'Matching securely...' : 'Confirm Deposit Match'}</button></div></section></div>}
    {showMissingPayment&&matching&&<div className="modal-overlay" style={{zIndex:1100}} onClick={()=>setShowMissingPayment(false)}><div onClick={event=>event.stopPropagation()} style={{width:'min(940px,94vw)',maxHeight:'94vh',overflowY:'auto'}}><OnlinePaymentForm defaultAmount={matching.amount} defaultDate={candidateWindow.endDate||matching.date} defaultSourceAccountId="sys-undeposited-funds" forceApproved onCancel={()=>setShowMissingPayment(false)} onCreated={(_status,item)=>{setShowMissingPayment(false);if(item?.id){setSelectedIds([item.id]);void matchDepositPayments([item.id]);}else{setNotice('Payment saved. Select it below to finish the deposit match.');void loadDepositCandidates(matching,candidateWindow.startDate,candidateWindow.endDate);}}}/></div></div>}
    {outgoing&&!showFullExpense && <div className="modal-overlay" onClick={()=>setOutgoing(null)}><section className="card" onClick={event=>event.stopPropagation()} style={{padding:22,border:'2px solid var(--green)',width:'min(760px,94vw)',maxHeight:'90vh',overflowY:'auto',background:'#fff',boxShadow:'0 24px 70px rgba(15,23,42,.3)'}}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 16 }}><div><h2 style={{ color: 'var(--navy)', margin: 0 }}>Match money out: ${Math.abs(outgoing.amount).toFixed(2)}</h2><div style={{ color: 'var(--text-muted)', marginTop: 4 }}>{outgoing.date} · {outgoing.description}</div></div><button className="btn btn-ghost" onClick={() => setOutgoing(null)}>Close</button></div>
      <label className="form-group"><span>What is this transaction?</span><select value={outgoingAction} onChange={e => { const action=e.target.value as 'expense'|'existing_bill'|'transfer';setOutgoingAction(action);setShowFullExpense(action==='expense');setError(''); }}><option value="expense">New expense</option><option value="existing_bill">Match existing bill</option><option value="transfer">Transfer to another account</option></select></label>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 18 }}><button className="btn btn-secondary" onClick={() => setOutgoing(null)}>Cancel</button>{outgoingAction!=='expense'&&<button className="btn btn-primary" disabled={saving} onClick={() => void confirmOutgoing()}>{saving ? 'Matching securely...' : 'Confirm Match'}</button>}</div>
    </section></div>}
    {showFullExpense&&outgoing&&<CloudTransactionModal mode="add" initialData={{id:outgoing.id,revision:1,vendor:outgoing.description,amount:Math.abs(outgoing.amount),dueDate:outgoing.date,status:'paid',sourceAccountId:selectedBank,memo:outgoing.description}} bankMatch={{accountId:selectedBank,bankTransactionId:outgoing.id,bankDate:outgoing.date,description:outgoing.description,amount:Math.abs(outgoing.amount)}} onBankActionChange={action=>{setOutgoingAction(action);setShowFullExpense(action==='expense');setError('');}} onClose={()=>{setShowFullExpense(false);setOutgoing(null);}} onSaved={message=>{removeMatchedFromVisibleFeed(outgoing.id);setShowFullExpense(false);setOutgoing(null);setNotice(message);void Promise.all([loadState(true),loadFeed(true)]);}}/>}
