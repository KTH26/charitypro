...
export const OnlineBank: React.FC = () => {
  ...
  const confirmDeposit = async () => {
    ...
    try {
      const response = await fetch('/api/v3/bank/match-deposit', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': requestId }, body: JSON.stringify({ requestId, accountId: selectedBank, bankTransactionId: matching.id, bankDate: matching.date, candidateFrom:candidateWindow.startDate,candidateTo:candidateWindow.endDate,description: matching.description, amount: matching.amount, transactionIds: selectedIds }) });
      const data = await response.json();
  ...
  return <main style={{ minHeight: '100vh', background: 'var(--bg)', padding: 28, fontFamily: 'Inter, sans-serif' }}><div style={{ maxWidth: 1400, margin: '0 auto' }}>
    ...
    {showMissingPayment&&matching&&<div className="modal-overlay" style={{zIndex:1100}} onClick={()=>setShowMissingPayment(false)}><div onClick={event=>event.stopPropagation()} style={{width:'min(940px,94vw)',maxHeight:'94vh',overflowY:'auto'}}><OnlinePaymentForm defaultAmount={matching.amount} defaultDate={candidateWindow.endDate||matching.date} defaultSourceAccountId="sys-undeposited-funds" onCancel={()=>setShowMissingPayment(false)} onCreated={()=>{setShowMissingPayment(false);setNotice('Payment saved. It is now available for this deposit match.');void loadDepositCandidates(matching,candidateWindow.startDate,candidateWindow.endDate);}}/></div></div>}
