...
export const OnlineBank: React.FC = () => {
  ...
  return <main style={{ minHeight: '100vh', background: 'var(--bg)', padding: 28, fontFamily: 'Inter, sans-serif' }}><div style={{ maxWidth: 1400, margin: '0 auto' }}>
    ...
    {accounts.length > 0 && <section className="card" style={{ padding: 18, marginBottom: 18 }}><div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px,1fr) 190px auto', gap: 10 }}><select value={selectedBank} onChange={e => { setSelectedBank(e.target.value); setFeedPage(1); }}>{accounts.map(account => <option key={account.id} value={account.id}>{account.name}{account.bankConnected ? '' : ' — reconnect required'}</option>)}</select><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} title="Optional: choose an earlier date for this sync" /><button className="btn btn-primary" onClick={() => void syncFeed()} disabled={loading || !selectedAccount?.bankConnected}>{loading ? 'Syncing...' : 'Sync New Bank Transactions'}</button></div><div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>Saved transactions load from CharityPro automatically. Sync starts from {startDate ? `your selected date (${startDate})` : lastSyncDate ? `the last successful sync (${lastSyncDate})` : 'the last 30 days for this first sync'}.</div></section>}
