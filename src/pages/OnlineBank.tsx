        ...
        {matching && (
          ...
          >
            ...
            >
              ...
              {depositAction === "transfer"&&<div style={{display:'grid',gap:16,marginTop:16}}><label className="form-group" style={{margin:0}}><span>Payment made from bank account *</span><select value={incomingTransferId} onChange={event=>setIncomingTransferId(event.target.value)}><option value="">Select matching bank payment</option>{incomingTransferCandidates.map(item=><option key={item.id} value={item.id}>{item.sourceName} — ${Number(item.amount).toFixed(2)} — {item.date}{(item.notes||item.description)?` — ${item.notes||item.description}`:''}{item.candidateKind==='bankFeed'?' — Unmatched bank transaction':''}</option>)}</select></label>{!incomingTransferCandidates.length&&<div style={{color:'var(--text-muted)'}}>No matching outgoing payment was found in any saved bank feed or existing transfer within 31 days.</div>}<div style={{display:'flex',justifyContent:'flex-end',gap:10}}><button className="btn btn-secondary" onClick={()=>setMatching(null)}>Cancel</button><button className="btn btn-primary" disabled={!incomingTransferId||saving} onClick={()=>void confirmIncomingTransfer()}>{saving?'Matching securely...':'Match Credit-Card Payment'}</button></div></div>}
            </section>
          </div>
        )}
