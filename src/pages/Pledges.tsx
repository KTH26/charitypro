...
export const Pledges: React.FC = () => {
  ...
  const pledgesWithBalance = useMemo(() => {
    return pledges.map(p => {
      // We need to fetch donor's other pledges and transactions to accurately compute the stats
      const donorPledges = pledges.filter(x => x.donorId === p.donorId).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
      const donorTransactions = transactions.filter(t => t.donorId === p.donorId);
      const donorRecurring = useStore.getState().recurringPayments.filter(r => r.donorId === p.donorId);
      
      const stats = getPledgeStats(p, donorPledges, donorTransactions, donorRecurring);
        
      return { ...p, openBalance: stats.balance, scheduledAmount: stats.scheduled };
    });
