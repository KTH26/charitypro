...
import { Users, User, FileText, Download, Plus, Check, Trash2, Pencil, X } from 'lucide-react';

export const Payroll: React.FC = () => {
  const { employees, fundraisers, t4aSlips, addEmployee, payPayrollEntity, addT4A, editT4A, deleteT4A, accruePayroll, bills, accounts, addBill, markBillPaid, editEmployee, deleteEmployee } = useStore();
  ...
  const [editingT4A, setEditingT4A] = useState<{ id: string, year: number, box48Amount: number } | null>(null);
  const [showEditLedgerEntry, setShowEditLedgerEntry] = useState<any | null>(null);
  ...
  const handlePay = (e: React.FormEvent) => {
    ...
    // Create a paid payroll bill (tagged isPayroll so it stays on Payroll tab, not Expenses)
    const billId = addBill({
      vendor: `Payroll: ${payTarget.name}`,
      employeeId: payTarget.type === 'employee' ? payTarget.id : undefined,
      amount,
    ...
    markBillPaid(billId, paySourceAccount);

    // Reduce the employee/fundraiser's owed balance (can go negative = credit)
    payPayrollEntity(payTarget.id, payTarget.type, amount);
  ...
  const handleAccrue = (e: React.FormEvent) => {
    ...
    if (isRecurring) {
      ...
      accruePayroll(accrueTarget.id, accrueTarget.type, parseFloat(accrueAmount), earningType, false, accrueDate);
    }
  ...
  return (
    <div style={{ display: 'grid', gap: '24px' }}>
                  ...
                  <tbody>
                    {employees.map(e => (
                      <tr key={e.id} onClick={() => setShowLedger({ id: e.id, type: 'employee', name: e.name })} style={{ cursor: 'pointer', transition: 'background 0.2s' }} className="hover-bg-input">
                        ...
                        <td>{e.role}</td>
                        <td>{e.email || '-'}</td>
                        <td style={{ fontWeight: 700, color: e.balanceOwed < 0 ? 'var(--green)' : 'var(--red)' }}>
                          {e.balanceOwed < 0
                            ? `(Credit: $${Math.abs(e.balanceOwed).toFixed(2)})`
                            : `$${e.balanceOwed.toFixed(2)}`}
                        </td>
                        <td>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            ...
                            <button className="btn btn-ghost btn-sm" onClick={(ev) => { 
                              ...
                              const t4aSum = bills.filter(b => (b.vendor === `Payroll: ${e.name}` || (b.isPayrollExpense && b.employeeId === e.id)) && b.t4aEligible && b.dueDate.startsWith(t4aYear.toString())).reduce((s, b) => s + b.amount, 0);
              ...
              <div className="table-container">
                <table>
                  ...
                  <tbody>
                          ...
                          <td style={{ textAlign: 'right' }}>
                            <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                              <button
                                ...
                                onClick={() => setEditingT4A({ id: t.id, year: t.year, box48Amount: t.box48Amount })}
                                title="Edit T4A"
                              >
                                <Pencil size={14} />
                              </button>
                              <button
                                className="btn btn-ghost btn-sm"
                                onClick={() => { if (confirm(`Delete T4A for ${entityName} (${t.year})?`)) deleteT4A(t.id); }}
                                style={{ color: 'var(--red)' }}
                                title="Delete T4A"
                              >
                                <Trash2 size={14} />
                    ...
                    {t4aSlips.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '20px' }}>No T4A slips generated yet.</td></tr>}
                  </tbody>
                </table>
              </div>
            </>
        ...
        <div className="modal-overlay" onClick={() => setShowAccrue(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAccrue} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div className="form-group">
                <label>Date</label>
                <input type="date" required value={accrueDate} onChange={e => setAccrueDate(e.target.value)} />
              </div>
              <div className="form-group">
            ...
            <div className="modal-body">
              <div className="table-container">
                <table>
                  ...
                  <tbody>
                    {bills.filter(b => b.vendor === `Payroll: ${showLedger.name}` || (b.isPayrollExpense && b.employeeId === showLedger.id)).sort((a, b) => new Date(b.dueDate).getTime() - new Date(a.dueDate).getTime()).map(b => (
                      <tr key={b.id}>
                    ...
                    {bills.filter(b => b.vendor === `Payroll: ${showLedger.name}` || (b.isPayrollExpense && b.employeeId === showLedger.id)).length === 0 && (
      ...
      {/* Edit T4A Modal */}
      {editingT4A && (
        <div className="modal-overlay" onClick={() => setEditingT4A(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Edit T4A Slip</h2>
              <button className="modal-close" onClick={() => setEditingT4A(null)}>✕</button>
            </div>
            <form
              onSubmit={e => {
                e.preventDefault();
                editT4A(editingT4A.id, { year: editingT4A.year, box48Amount: editingT4A.box48Amount });
                setEditingT4A(null);
              }}
            ...
            >
              <div className="form-group">
                ...
                <input
                  ...
                  max="2100"
                  required
                  value={editingT4A.year}
                  onChange={e => setEditingT4A({ ...editingT4A, year: parseInt(e.target.value) })}
                />
              </div>
              <div className="form-group">
                <label>Box 48 — Fees for Services ($)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
                  required
                  value={editingT4A.box48Amount}
                  onChange={e => setEditingT4A({ ...editingT4A, box48Amount: parseFloat(e.target.value) })}
                />
              </div>
              <div className="modal-footer" style={{ marginTop: '8px' }}>
                <button type="button" className="btn btn-secondary" onClick={() => setEditingT4A(null)}>Cancel</button>
                <button type="submit" className="btn btn-primary">Save Changes</button>
              </div>
            </form>
          </div>
      ...
      {showEditLedgerEntry && (
        <div className="modal-overlay" onClick={() => setShowEditLedgerEntry(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Edit Ledger Entry</h2>
              <button className="modal-close" onClick={() => setShowEditLedgerEntry(null)}><X size={20} /></button>
            </div>
            <form onSubmit={(e) => {
              e.preventDefault();
              editBill(showEditLedgerEntry.id, { 
                amount: showEditLedgerEntry.amount, 
                dueDate: showEditLedgerEntry.dueDate,
                paidDate: showEditLedgerEntry.paidDate,
                memo: showEditLedgerEntry.memo 
              });
              setShowEditLedgerEntry(null);
            }} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div className="form-group">
                <label>Date</label>
                <input 
                  type="date" 
                  required 
                  value={showEditLedgerEntry.status === 'paid' ? (showEditLedgerEntry.paidDate || '') : showEditLedgerEntry.dueDate} 
                  onChange={e => setShowEditLedgerEntry({
                    ...showEditLedgerEntry, 
                    [showEditLedgerEntry.status === 'paid' ? 'paidDate' : 'dueDate']: e.target.value
                  })} 
                />
              </div>
              <div className="form-group">
                ...
                <input 
                  ...
                  step="0.01" 
                  required 
                  value={showEditLedgerEntry.amount} 
                  onChange={e => setShowEditLedgerEntry({...showEditLedgerEntry, amount: parseFloat(e.target.value) || 0})} 
                />
              </div>
              <div className="form-group">
                ...
                <input 
                  type="text" 
                  value={showEditLedgerEntry.memo || ''} 
                  onChange={e => setShowEditLedgerEntry({...showEditLedgerEntry, memo: e.target.value})} 
                />
              </div>
              <div className="modal-footer" style={{ marginTop: '24px' }}>
                <button type="button" className="btn btn-secondary" onClick={() => setShowEditLedgerEntry(null)}>Cancel</button>
                <button type="submit" className="btn btn-primary">Save Changes</button>
              </div>
            </form>
          </div>
