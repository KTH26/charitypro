...
import { Search, Play, Pause, Trash2, Pencil } from 'lucide-react';
import { useT } from '../i18n';

export const Schedules: React.FC = () => {
  const { recurringPayments, donors, toggleRecurring, deleteRecurring, deleteAllRecurring, editRecurring, isRtl } = useStore();
  const T = useT(isRtl);
  ...
  const [editingSchedule, setEditingSchedule] = useState<EditState | null>(null);
  ...
  const openEdit = (s: typeof recurringPayments[0]) => {
    setEditingSchedule({
      id: s.id,
  ...
  const handleEditSave = (e: React.FormEvent) => {
    ...
    editRecurring(editingSchedule.id, {
      amount: parseFloat(editingSchedule.amount),
      frequency: editingSchedule.frequency,
      nextDate: editingSchedule.nextDate,
      endDate: editingSchedule.endDate || undefined,
      method: editingSchedule.method as any,
    });
                  ...
                  <tr key={schedule.id} style={{ opacity: schedule.active ? 1 : 0.6 }}>
                    ...
                    <td style={{ fontWeight: 600 }}>
                      {(() => {
                        ...
                        const yiddishName = [donor.hebFirstName, donor.hebLastName].filter(Boolean).join(' ');
                        return yiddishName ? `${donor.name} (${yiddishName})` : donor.name;
                      })()}
                    </td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => openEdit(schedule)} title="Edit schedule">
                          <Pencil size={14} />
                        </button>
                        <button className="btn btn-secondary btn-sm" onClick={() => toggleRecurring(schedule.id)} title={schedule.active ? 'Pause' : 'Resume'}>
                          {schedule.active ? <Pause size={14} /> : <Play size={14} />}
                        </button>
                        <button className="btn btn-ghost btn-sm" style={{ color: 'var(--red)' }} onClick={() => handleDeleteSingle(schedule.id)} title="Delete">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
            ...
            <form onSubmit={handleEditSave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={e => setEditingSchedule({ ...editingSchedule, amount: e.target.value })}
                />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={e => setEditingSchedule({ ...editingSchedule, method: e.target.value })}
                >
