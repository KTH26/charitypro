...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { isRtl, donors, tasks, accounts, syncConflicts, dismissConflict } = useStore();
    ...
    <div className="app-container" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <main className="main-content">
        ...
        {syncConflicts.length > 0 && (
          <div style={{ margin: '20px 20px 0', padding: '12px 20px', background: 'var(--red-bg)', color: 'var(--red)', borderRadius: '12px', border: '1px solid rgba(239, 68, 68, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontFamily: 'Outfit, sans-serif' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ background: 'var(--red)', color: 'white', borderRadius: '50%', width: 24, height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>!</div>
              <div>
                <strong>{syncConflicts.length} Sync Conflict(s) Detected!</strong>
                <div style={{ fontSize: '0.85rem', opacity: 0.85 }}>Your offline edits were rejected because newer data exists on the server. Please check the console for details.</div>
              </div>
            </div>
            <button 
              onClick={() => syncConflicts.forEach(c => dismissConflict(c.id))}
              style={{ padding: '6px 12px', background: 'var(--red)', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem' }}
            >
              Dismiss All
            </button>
          </div>
