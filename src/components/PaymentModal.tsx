...
export const PaymentModal: React.FC<Props> = ({ donorId, onClose }) => {
  ...
  const handleOneTime = async () => {
    ...
    if (method === 'credit_card' && !bypassGateway) {
      if (!solaApiKey) {
  ...
  const handleRecurring = async () => {
    ...
    if (recMethod === 'credit_card' && !bypassGateway) {
      if (!solaApiKey) {
        setError('Sola API Key is missing. Please configure it in settings.');
        return;
      }
                ...
                <div style={{ display: 'grid', gap: '20px' }}>
                  ...
                  {method === 'credit_card' && (
                    <div style={{ background: 'var(--bg-input)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border)' }}>
                      ...
                      <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', marginTop: '16px', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                        <input type="checkbox" checked={bypassGateway} onChange={e => setBypassGateway(e.target.checked)} />
                        Record external charge only (skip Sola gateway)
                      </label>
                    </div>
                  )}
                ...
                <div style={{ display: 'grid', gap: '20px' }}>
                  ...
                  {recCurrency === 'USD' && (
                    <div style={{ background: 'var(--yellow-bg)', border: '1px solid rgba(245, 158, 11, 0.2)', padding: '16px', borderRadius: '12px' }}>
                      ...
                      <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                        Equivalent: <strong style={{ color: 'var(--navy)' }}>${getAmountCAD(recAmount).toFixed(2)} CAD</strong> per installment.
                      </div>
                    </div>
                  )}
